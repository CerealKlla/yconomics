...
Design settled but not yet built: the dropped-item bag (design doc Section 4) — every dropped item including death drops (which will now include the Coin Purse's 10% death-drop and hostile mobs' nugget drops), 3-block clustering, right-click-only chest-style container, no capacity cap, despawns on the normal item-entity timer. Also newly flagged, ownership undecided: an NPC gossip/paid-tips mechanic (Section 5) that touches Lyfe's Cartographyr-skill `KnowledgeFactor` system.

Next: **build the dropped-item bag** (user's explicit next task), then **live confirmation of all of it together** via `runClient` — the gambling recipe, the full currency swap, hostile mob drops, the Coin Purse, and the bag, all in one test pass. The gossip mechanic's mod ownership needs deciding before it can be built at all.
