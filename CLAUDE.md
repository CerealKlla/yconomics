...
**Coin Purse now shows tier and nugget count without hovering, 2026-09-26** (see [context/decisions.md](context/decisions.md)) — new `currency.client.CoinPurseDecorator`, using NeoForge's `IItemDecorator`/`RegisterItemDecorationsEvent` (confirmed against the decompiled source to be the real hook for this, not an override of vanilla's own stack-count text, which has no such hook in this version). Draws "T\<tier\>" top-left and the nugget total bottom-right directly on the purse's inventory-slot icon. `./gradlew build` green; **live confirmation is still pending.**

Next: **live confirmation of the bag's UI polish**, the Coin Purse tier/count decorator, and the new Merchant skill's price bonus/XP (see lyfe's CLAUDE.md) — then a full pass over everything built this session together — the gambling recipe, the full currency swap, hostile mob drops, the Coin Purse, and the dropped-item bag (the user's own stated test plan).
