...
**Dropped-item bag built** (design doc Section 4) — `bag.LootBagEntity`, a world entity (not a block) holding a 54-slot container, reusing vanilla's own `ChestMenu.sixRows`/generic-6-row screen wholesale (no custom menu type or client screen code needed at all). `bag.LootBagListener` handles capture: voluntary drops are watched via `ItemTossEvent` and swept in once they actually touch the ground (`ServerTickEvent.Post` checking `onGround()`), while death drops are captured directly from the inventory at `LivingDeathEvent` (before vanilla's own drop logic runs), skipping the fall entirely. Clustering is a 3-block-radius search for an existing bag before creating a new one. 54 slots isn't literally unbounded (a real unbounded inventory needs a custom scrolling screen this pass doesn't build), but the design's own "if there's no room, scatter on the ground" fallback covers that exactly. Boot-smoke-tested clean, no bugs caught this time. **Live confirmation is still pending.**
...
Next: **live confirmation of everything built today, all together, in one test pass** via `runClient` — the gambling recipe, the full currency swap, hostile mob drops, the Coin Purse, and the dropped-item bag. This is the user's own stated plan (test all three of today's new mechanics — mob drops, Coin Purse, bag — at once).
